//! Domain policy for Cart aggregate
//!
...
/// Domain policy for Cart — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Cart>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type CartDomainPolicy = PermitAllPolicy<Cart>;
