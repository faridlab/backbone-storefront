//! Domain policy for CartLine aggregate
//!
...
/// Domain policy for CartLine — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<CartLine>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type CartLineDomainPolicy = PermitAllPolicy<CartLine>;
