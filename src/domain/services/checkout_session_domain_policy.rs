//! Domain policy for CheckoutSession aggregate
//!
...
/// Domain policy for CheckoutSession — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<CheckoutSession>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type CheckoutSessionDomainPolicy = PermitAllPolicy<CheckoutSession>;
