//! Domain policy for StorefrontAuditLog aggregate
//!
...
/// Domain policy for StorefrontAuditLog — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<StorefrontAuditLog>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type StorefrontAuditLogDomainPolicy = PermitAllPolicy<StorefrontAuditLog>;
