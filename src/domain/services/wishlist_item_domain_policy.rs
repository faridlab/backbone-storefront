//! Domain policy for WishlistItem aggregate
//!
...
/// Domain policy for WishlistItem — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<WishlistItem>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type WishlistItemDomainPolicy = PermitAllPolicy<WishlistItem>;
